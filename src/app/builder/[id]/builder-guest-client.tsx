...
export default function BuilderGuestClient({ id }: { id: string }) {
  ...
  useEffect(() => {
    const loaded = loadGuestResume(id);
    setData(loaded);
  ...
  const generateFullResumeAI = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        alert("AI full resume generation failed. Check server logs.");
        return;
      }
      ...
      setData(json.data);
      saveGuestResume(id, json.data);

      alert("Resume generated. Review your content.");
    } finally {
      setGenLoading(false);
    }
  };
  ...
  const openGuestPdf = () => {
    window.open(`/print/guest/${id}`, "_blank", "noreferrer");
  };
            ...
            <div className="flex items-center justify-between gap-4 flex-wrap">
              <div>
                <h2 className="text-xl font-bold bg-gradient-to-r from-cyan-600 to-fuchsia-600 bg-clip-text text-transparent">
                  Resume Builder (Guest)
                </h2>
                <p className="text-xs text-slate-600 mt-1">
                  Auto-saves • Template: {templateId}
                </p>
              </div>
              ...
              <div className="flex gap-2 flex-wrap justify-end">
                <Link
                  ...
                  href={`/builder/${id}/template?mode=guest`}
                >
                  Templates
                </Link>
                ...
                <button
                  type="button"
                  onClick={openGuestPdf}
                  className="rounded-xl bg-black px-4 py-2 text-sm font-bold text-white hover:opacity-90 transition"
                >
                  Save as PDF
                </button>
              </div>
                  ...
                  <div key={idx} className="rounded-2xl border border-black/10 bg-white/60 p-4">
                    ...
                    <div className="mt-3 grid gap-2">
                      ...
                      <textarea
                        ...
                        placeholder="Bullets / achievements"
                      />
                    </div>
                  </div>
                ))}
